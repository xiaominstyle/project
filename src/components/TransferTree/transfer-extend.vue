      ...
      <div class="transfer-right">
        ...
        <div class="transfer-right-item" :class="{'transfer-right-small':!move_up}">
          <h3 class="transfer-title">
            ...
            <img class="move_up_img" v-else src="../../assets/img/shang.png" alt="" @click="moveUp('up')">
          </h3>
          <!-- 内容区 -->
          <div class="transfer-main" v-if='move_up'>
            <!-- <slot name='to'></slot> -->
  ...
  methods: {
    ...
    addToAims() {
      ...
      newSkeletonHalfCheckedNodes.forEach(item => {
        item[children__] = [];
        if (item[parentId__] == 0) {
          this.$refs["to-tree"].append(item);
        } else {
          this.$refs["to-tree"].append(item, item[parentId__]);
        }
      });
    ...
    removeToSource() {
      ...
      newSkeletonHalfCheckedNodes.forEach(item => {
        ...
        if (item[parentId__] == 0) {
          ...
          this.$refs["from-tree"].append(item, item[parentId__]);
        }
      });
  ...
  watch: {
    ...
    from_check_keys(val) {
      if (val.length > 0) {
        ...
        let allCheck = val.filter(item => item[this.parentId] == 0);
